...
df[,c(15:ncol(df))][sapply(df[,c(15:ncol(df))], is.character)] <- lapply(df[,c(15:ncol(df))][sapply(df[,c(15:ncol(df))], is.character)], 
                                       as.factor)
df[,is.factor(df[,15:ncol(df)])]  <- df[,is.factor(df[,15:ncol(df)])]  %>% 
  mutate_all(fct_explicit_na, "NA")

df <- df %>%
  dplyr::select(c('public_client_id','bowel','sex','age','BMI_CALC','eGFR'), everything())
df
```


```{r}
if (any(str_detect(substr(newlabels[1],4,nchar(newlabels[1])),paste0(c("\\+ times","hours","Rarely","min","Several","Never","Most","Occasionally","Strongly","I drink","Once","I don't","Less than","times","ero","ore","isagree","Agree","\\bagree","Daily","Often","1-1"))),
        str_detect(substr(newlabels[1],4,nchar(newlabels[1])),paste0(c("\\+ times","hours","Rarely","min","Several","Never","Most","Occasionally","Strongly","I drink","Once","I don't","Less than","times","ero","ore","isagree","Agree","\\bagree","Daily","Often","1-1"))),
        str_detect(substr(newlabels[1],4,nchar(newlabels[1])),paste0(c("\\+ times","hours","Rarely","min","Several","Never","Most","Occasionally","Strongly","I drink","Once","I don't","Less than","times","ero","ore","isagree","Agree","\\bagree","Daily","Often","1-1"))))) {
    order = TRUE
  }
```
...
for (col in colnames(test)[13:ncol(test)]) {
  ...
  if (is.factor(df[,col]) & col != 'month' & col != 'season' & col != 'weekday') {
    ...
    print(newlabels)
    if (any(str_detect(substr(newlabels[1],4,nchar(newlabels[1])),paste0(c("\\+ times","hours","Rarely","min","Several","Never","Most","Occasionally","Strongly","I drink","Once","I don't","Less than","times","ero","ore","isagree","Agree","\\bagree","Daily","Often","1-1"))))
          | any(str_detect(substr(newlabels[2],4,nchar(newlabels[2])),paste0(c("\\+ times","hours","Rarely","min","Several","Never","Most","Occasionally","Strongly","I drink","Once","I don't","Less than","times","ero","ore","isagree","Agree","\\bagree","Daily","Often","1-1"))))) {
      order = TRUE
    }
    print("Reversing numbering")
    new <- reverse(newlabels,newlevels)
    newlabels <- new[[1]]
    newlevels <- new[[2]]
    if (any(str_detect(substr(newlabels[1],2,2),c(paste0(c(3:10)))))) {
      print("Reversing order")
      newlevels <- rev(newlevels)
      newlabels <- rev(newlabels)
    }
    if (any(str_detect(substr(newlabels[1],4,nchar(newlabels[1])),paste0(c("\\+ times","I drink","ore","Agree","\\bagree","aily","ften","1-1"))))) {
      print("Reversing order")
      newlevels <- rev(newlevels)
      newlabels <- rev(newlabels)
      if (any(str_detect(substr(newlabels[1],2,2),c(paste0(c(3:10)))))) {
        print("Reversing numbering again")
        new <- reverse(newlabels,newlevels)
        newlabels <- new[[1]]
        newlevels <- new[[2]]
      }
    ...
    replace_levels<-newlevels
    replace_labels<-oldlevels
    if (substr(oldlabels[1],4,nchar(oldlabels[1])) != substr(newlabels[1],4,nchar(newlabels[1]))) {
      replace_levels<-rev(replace_levels)
      replace_labels<-rev(replace_labels)
      if (any(str_detect(substr(oldlabels[1],2,2),as.character(c(0:2)))) ==  any(str_detect(substr(newlabels[1],2,2),as.character(c(0:2))))) {
        replace_levels<-rev(replace_levels)
      }
    }
    test[,col]<-factor(test[,col],levels=levels(test[,col]),labels=oldlevels, ordered = order)
    test[,col]<-factor(test[,col],levels=replace_levels,labels=replace_labels, ordered = order)
    test[,col]<-factor(test[,col],levels=replace_levels,labels=newlabels, ordered = order)
  } else {
...
str(df[,'ab_pain'])
str(test[,'ab_pain'])
str(df[,'assessment.joyful.moments.feel_empowered.enum'])
str(test[,'assessment.joyful.moments.feel_empowered.enum'])
str(df[,'assessment.satisfaction.rarely_rested.enum'])
str(test[,'assessment.satisfaction.rarely_rested.enum'])
str(df[,'assessment.personality.irritated_easily.enum'])
str(test[,'assessment.personality.irritated_easily.enum'])
str(df[,'assessment.satisfaction.rarely_rested.enum'])
str(test[,'assessment.satisfaction.rarely_rested.enum'])
str(df[,'assessment.personality.often_blue.enum'])
str(test[,'assessment.personality.often_blue.enum'])
str(df[,'assessment.lifestyle.vegetables.enum'])
str(test[,'assessment.lifestyle.vegetables.enum'])
str(df[,'assessment.lifestyle.water.enum'])
str(test[,'assessment.lifestyle.water.enum'])
str(df[,'assessment.lifestyle.cruciferous_vegetables.enum'])
str(test[,'assessment.lifestyle.cruciferous_vegetables.enum'])
str(df[,'assessment.lifestyle.breakfast.enum'])
str(test[,'assessment.lifestyle.breakfast.enum'])
str(df[,'assessment.diet.freq_cook_home.int'])
str(test[,'assessment.diet.freq_cook_home.int'])
str(df[,'assessment.diet.freq_raw_vegetables_salad.int'])
str(test[,'assessment.diet.freq_raw_vegetables_salad.int'])
str(df[,'assessment.lifestyle.sugary_drinks.enum'])
str(test[,'assessment.lifestyle.sugary_drinks.enum'])
str(df[,'daily_fruit'])
str(test[,'daily_fruit'])
str(df[,'exercise'])
str(test[,'exercise'])
str(df[,'assessment.lifestyle.alcohol_days_a_week.enum'])
str(test[,'assessment.lifestyle.alcohol_days_a_week.enum'])
str(df[,'assessment.digestion.bowel.movement.completion.enum'])
str(test[,'assessment.digestion.bowel.movement.completion.enum'])
str(df[,'laxatives'])
str(test[,'laxatives'])
str(df[,'assessment.digestion.diarrhea.enum'])
str(test[,'assessment.digestion.diarrhea.enum'])
str(df[,'assessment.digestion.bloating.enum'])
str(test[,'assessment.digestion.bloating.enum'])
str(df[,'assessment.health.history.drugs_cholesterol.int'])
str(test[,'assessment.health.history.drugs_cholesterol.int'])
str(df[,'assessment.digestion.poor_or_lack_of_appetite.enum'])
str(test[,'assessment.digestion.poor_or_lack_of_appetite.enum'])
str(df[,'assessment.digestion.gas.enum'])
str(test[,'assessment.digestion.gas.enum'])

str(df[,'assessment.lifestyle.snack.enum'])
str(test[,'assessment.lifestyle.snack.enum'])

str(df[,'assessment.diet.freq_breakfast.int'])
str(test[,'assessment.diet.freq_breakfast.int'])
...
test_final <- merge(test[,1:15],test_filt)
...
test_final <- merge(test_final[,1:15],test_reduced)
test_final <- test_final %>%
  dplyr::select(c('public_client_id','bowel','sex','age','BMI_CALC','eGFR'), everything())
test_final$bowel <- factor(test_final$bowel, levels = c("Constipation","Low Normal","High Normal","Diarrhea"), labels = c("Constipation","Low Normal","High Normal","Diarrhea"), ordered = TRUE)
...
```{r}
df2 <- df
my_names <- names(df2)[-which((colnames(df2) == 'public_client_id') | (colnames(df2) == 'bowel') | (colnames(df2) == 'BMI_CALC') | (colnames(df2) == 'sex') | (colnames(df2) == 'age') | (colnames(df2) == 'eGFR') | (colnames(df2) == 'pss_4') | (colnames(df2) == 'pss_1') | (colnames(df2) == 'pss_2') | (colnames(df2) == 'pss_3') | (colnames(df2) == 'days_since_first_call') | (colnames(df2) == 'days_in_program') | (colnames(df2) == 'days_since_first_draw'))]
...
full <- ggplot(df_full, aes(x = value, y = factor(variables, levels = c("Age","Sex","BMI",'GFR',"BMF diarrhea","BMF low normal", "BMF constipated")))) +
  ...
  scale_y_discrete(name="Regression Variable", labels = c(
    ...
    "Sex" = expression("Female"*symbol('\253')*"Male"),
    "BMI" = expression("Low BMI"*symbol('\253')*"High BMI"),
    "GFR" = expression("Low GFR"*symbol('\253')*"High GFR"),
    "BMF constipated" = expression("High Normal BMF"*symbol('\253')*"Constipation"),
    "BMF low normal" = expression("High Normal BMF"*symbol('\253')*"Low Normal BMF"),
    "BMF diarrhea" =  expression("High Normal BMF"*symbol('\253')*"Diarrhea")))
full
